...
describe('handleWebhookDelivery push routing', () => {
  it('ignores an already-queued push without creating work', async () => {
    const { deps } = fixture();
    const refresh = vi.fn().mockResolvedValue(undefined);
    deps.refreshGithubAnchorProjection = refresh;
    ...
    expect(result.status).toBe(200);
    expect(result.body).toEqual({ ignored: 'unhandled-event' });
    expect(await deps.store.listLiveRuns()).toEqual([]);
