...
#
# Broker-bound runs pass ONLY when an exact attempt-claim marker is found
...
EXPECTED_COMMENT_LOGIN="${EXPECTED_COMMENT_LOGIN:-}"
RUNBOOK="${RUNBOOK:-}"
EXCLUDE_PR_AUTHOR="${EXCLUDE_PR_AUTHOR:-}"
EXCLUDE_COMMENT_ID="${EXCLUDE_COMMENT_ID:-}"
# Optional for the Published surface: hosted/broker-bound workers always set
# it and therefore use exact-only validation. Standalone consumers without a
# broker leave it empty and must provide the legacy inference inputs instead.
ATTEMPT_ID="${ATTEMPT_ID:-}"

if [ -z "$ATTEMPT_ID" ]; then
  : "${STARTED_AT:?STARTED_AT is required when ATTEMPT_ID is unset}"
  : "${EXPECTED_COMMENT_LOGIN:?EXPECTED_COMMENT_LOGIN is required when ATTEMPT_ID is unset}"
fi
...
#
# Exact-mode lookup failures are recorded unconditionally. Legacy mode skips
# this block because ATTEMPT_ID is empty and uses clauses (a)-(e) below.
...
# ATTEMPT_ID is a hard security boundary for hosted workers: when present,
# never fall through to the weaker time-window/login inference below.
if [ -n "$ATTEMPT_ID" ]; then
  if [ -n "$found" ]; then
