...
function assertCanaryContracts({ ledger, generation }) {
  ...
  if (projection?.state !== "converged" || projection.desiredRevision !== projection.observedRevision) {
    throw new Error(
      `Canary projector contract failed: expected converged matching revisions, got ${projection ? JSON.stringify(projection) : "no projection status"}.`
    );
  }
}
