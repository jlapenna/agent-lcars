  ...
  'node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/schema/yaml-1.1/merge.js'(
    ...
    function addMergeToJSMap(ctx, map, value) {
      ...
      if (identity.isSeq(source))
        for (const it of source.items) mergeValue(ctx, map, it);
    ...
    function resolveAliasValue(ctx, value) {
      return ctx && identity.isAlias(value)
...
// .github/actions/validate-worker-workflows/validate.cjs
var fs = require('node:fs');
var path = require('node:path');
var { parse: parseYaml } = require_dist();
...
var ANCHOR_UNION = "(inputs.issue!=''||inputs.work!='')";
...
function validateWorkflow(provider, file, document) {
  ...
  if (missingInputs.length > 0 || unexpectedInputs.length > 0) {
    errors.push(
      `${file}: workflow_dispatch input set mismatch${missingInputs.length > 0 ? `; missing ${missingInputs.join(', ')}` : ''}${unexpectedInputs.length > 0 ? `; unexpected ${unexpectedInputs.join(', ')}` : ''}`,
    );
  }
  ...
  if (document.concurrency !== void 0) {
