...
# Assert the FULL permissions block, not just id-token: write's presence --
# a plain grep for that string would still pass if e.g. `contents: write`
# were added alongside it. Every "*: write" line in the block must be
# id-token's.
