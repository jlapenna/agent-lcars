...
| A task looks stuck (no run, no comment, no progress) for a long time                                                  | Orchestrator (reconciliation)                 | `dispatch-reconcile.yml`'s run history, then `/api/control-plane/reconcile`'s response body (`lost`/`terminal`/`retried`/`dispatched`/`reported`)           |
