...
| Ledger stuck on the same revision for a long time, no anomaly recorded                                                                                                                  | Dispatch controller (reconciliation) — **but check Runner platform first**: `dispatch-reconcile.yml` itself runs on `CONTROL_PLANE_RUNNER_LABEL`, so a Runner-platform outage on that pool silently takes out the repair mechanism too | `gh api repos/<owner>/<repo>/actions/runners` again, before assuming the reconciler is merely slow                                              |
...
- `.github/workflows/dispatch-reconcile.yml` — the `scan` job, same pool,
  every 30 minutes.
...
It also depends on the same `CONTROL_PLANE_RUNNER_LABEL` pool as
`agent-router.yml`, so a Runner-platform outage on that pool disables the
safety net at the same time it disables the thing it's meant to repair (see
the Runner platform worked incident, and the symptom-table row about it).
...
never loaded a listener for that label. Every `agent-router.yml` and
`dispatch-reconcile.yml` job requesting that label queued forever and was
eventually evicted. Total outage: roughly 20 hours. Nothing alerted. The
