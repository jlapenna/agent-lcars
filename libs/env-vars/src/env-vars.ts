export interface EnvVars {
  ...
  // Octokit client (github-client.ts's getGithubClient()) and the outbox
  // drain's dispatch provider (createDispatchTokenProvider). There is no
  // more long-lived ambient PAT fallback; construction fails fast if
  // either is unset.
   ...
   * `session-metadata/` (titles an agent declared via `lcars session title`). See
