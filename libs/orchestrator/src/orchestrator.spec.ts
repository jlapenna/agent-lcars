...
describe('settling a run whose executor is already terminal (#1361)', () => {
  ...
  it("lets a task's queued pendingRequest beat the settle's own auto-retry", async () => {
    const { store, orchestrator } = fixture();
    const { run } = await started(orchestrator, 'req-1');
    await orchestrator.request({
      taskId: TASK,
      requestId: 'queued-by-operator',
      pipeline: 'codex',
      queueIfBusy: true,
    });

    const outcome = await orchestrator.settleTerminalRuns([
      { runId: run.runId, conclusion: 'timed_out' },
    ]);

    // The queued request was consumed atomically inside the settle, taking
    // the lock; the auto-retry request then found the task busy and was
    // refused -- exactly the documented precedence expireLease already has.
    expect(outcome.settled).toHaveLength(1);
    expect(outcome.retried).toEqual([]);
    const active = await store.readActiveRun(TASK);
    expect(active?.requestId).toBe('queued-by-operator');
    expect(active?.pipeline).toBe('codex');
  });
});
